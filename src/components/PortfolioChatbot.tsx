  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const errorMessage: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: "Sorry, I'm having trouble connecting right now. You can explore Daniyal's live portfolio sections directly or email him at **mdaniyalhayyat@gmail.com**.",
        timestamp: new Date()
      };
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="group relative flex items-center justify-center w-14 h-14 rounded-full bg-cyan-500 text-slate-950 shadow-xl shadow-cyan-500/25 hover:bg-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 dark:focus:ring-offset-slate-950 transition-all"
        ...
        >
          ...
          {/* Tooltip */}
          <span className="absolute right-full mr-3 px-3 py-1.5 bg-slate-900 dark:bg-slate-800 text-white text-xs font-medium rounded-lg shadow-lg opacity-0 group-hover:opacity-150 transition-opacity pointer-events-none whitespace-nowrap border border-slate-700/50">
            Ask Daniyal AI ✨
          </span>
        </motion.button>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 sm:right-6 z-50 w-[92vw] sm:w-[420px] h-[580px] max-h-[85vh] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden backdrop-blur-xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3.5 bg-slate-50 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-slate-950/40">
              ...
              {isLoading && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 px-4 py-3 rounded-2xl rounded-tl-none shadow-xs flex items-center gap-1.5">
            ...
            <div className="px-3 py-2 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 overflow-x-auto no-scrollbar flex items-center gap-1.5">
              {SUGGESTED_QUESTIONS.map((item, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSendMessage(item.query)}
                  disabled={isLoading}
                  className="shrink-0 px-3 py-1 rounded-full text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-cyan-500 hover:text-slate-950 dark:hover:bg-cyan-500 dark:hover:text-slate-950 transition-colors disabled:opacity-50"
                >
                  {item.label}
                </button>
              ))}
            ...
            <form
              ...
              className="flex items-center gap-2 p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800"
            >
              <input
                ref={inputRef}
                type="text"
              ...
              <button
                ...
                className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-cyan-500 text-slate-950 hover:bg-cyan-400 disabled:opacity-50 disabled:hover:bg-cyan-500 transition-colors shrink-0 shadow-sm"
              >
